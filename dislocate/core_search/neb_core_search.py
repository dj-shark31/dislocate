...
from dislocate.core_analysis.visualize_core import Core
import sys
import subprocess
import tempfile
from dislocate.core_search.ESPM import babel_make_dipole
...
def parse_arguments():
    ...
    parser.add_argument('--cij', type=float, nargs=5, default=[177.1, 84.8, 82.9, 193.8, 54.8],
                       help='Elastic constants (C11, C12, C13, C33, C44)')
    parser.add_argument('--reference_cell', type=str, required=True,
                       help='Reference cell structure file')
    return parser.parse_args()

def get_dislocation_positions(image, reference_cell):
    """
    Extract dislocation positions from output file generated by dislocate.core_analysis.analyze_core.
    
    Args:
        output_file (str): Path to output file containing dislocation positions from DXA analysis
        
    Returns:
        tuple: Two numpy arrays containing (x,y) coordinates of left and right dislocations
    """
    
    # Create temporary output file
    tmp_output = tempfile.NamedTemporaryFile(prefix='output-', suffix='.dat', delete=False).name
    
    # Create temporary POSCAR file for image
    tmp_poscar = tempfile.NamedTemporaryFile(prefix='POSCAR-', suffix='.poscar', delete=False).name
    write(tmp_poscar, image, format='vasp')
    subprocess.run([sys.executable, '-m', 'dislocate.core_analysis.analyze_core', '--ncore', '1', '--thickness', '1', 
    '--ref_cell', reference_cell, '--output_files', tmp_output, '--dis_cells', tmp_poscar, '--energy_stress', 'false', 
    '--fitting', 'true', '--ovito', 'false', '--nye', 'false'])
    
    #Extract dislocation positions from output file
    core = Core(tmp_output)

    # Clean up temporary files
    os.remove(tmp_output)
    os.remove(tmp_poscar)

    return core.left_dislocation_position_babel, core.right_dislocation_position_babel

def get_elastic_energy(image, reference_cell, cij):
    dislocation_positions = get_dislocation_positions(image, reference_cell)
    a = image.cell.lengths()[2]
    r11_i, r21_i = dislocation_positions[0]
    r12_i, r22_i = dislocation_positions[1]
    parameters = {
            "A0": str(a),
            "R11": str(r11_i),
            "R21": str(r21_i),
            ...
            "C33": str(cij[3]),
            "C44": str(cij[4]),
            "C66": str((cij[0] - cij[1]) / 2)
        }

    template_path = os.path.join(script_dir, "input.babel_S")
    energy = babel_make_dipole(template_path, parameters, get_elastic_energy=True)
    return energy

def get_elastic_energy_corrections(images, reference_cell, cij):
    elastic_energies = []
    for image in images:
        elastic_energies.append(get_elastic_energy(image, reference_cell, cij))

    return [e - elastic_energies[0] for e in elastic_energies]

def get_unique_state_energy(initial_energy, final_energy, intermediate_energies, energy_tolerance, elastic_corrections = None):
    unique_states = []

    if elastic_corrections is not None:
        final_energy = final_energy - elastic_corrections[-1]
        intermediate_energies = [e - elastic_corrections[i+1] for i, e in enumerate(intermediate_energies)]
...
def get_new_cell(initial_image, final_image, n_images, potential_path, neb_fmax=0.005, relax_fmax=0.001, energy_tolerance=0.001, device='cpu', center=None, radius=None, reference_cell=None, cij=None):
    """
    ...
    images = run_neb(initial_image, final_image, n_images, potential_path, neb_fmax=neb_fmax, device=device, center=center, radius=radius)
    ...
    if center is not None and radius is not None:
        elastic_corrections = get_elastic_energy_corrections(images, reference_cell, cij)
    else:
        elastic_corrections = None

    # Find intermediate states with different energies than endpoints
    different_energy_states = get_unique_state_energy(initial_energy, final_energy, intermediate_energies, energy_tolerance, elastic_corrections)
...
def iteration(current_initial, current_final, args, iteration):
    if iteration > args.max_iterations:
        ...
        new_cells = get_new_cell(current_initial, current_final, args.n_images, args.potential_path, neb_fmax=args.neb_fmax, 
        relax_fmax=args.relax_fmax, energy_tolerance=args.energy_tolerance, device=args.device, center=args.center, radius=args.radius, reference_cell=args.reference_cell, cij=args.cij)
